...
public class SavePastDailyTotalMovieProcessor implements ItemProcessor<KobisMovieRankResponseDto, List<MovieRank>> {
    @Override
    public List<MovieRank> process(KobisMovieRankResponseDto item) {
        ...
        return item.getBoxOfficeResult()
                ...
                .stream()
                .map(kobisBoxOfficeResponseDto -> MovieRank.createMovieRank(showRangeLocalDate, MovieRankType.TOTAL, kobisBoxOfficeResponseDto))
